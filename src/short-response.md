...
### Question 2
  ...
  The code logs `"Michael Jordan"` because objects are copied by **reference** and not by value. Since, when we make a copy of an object, we're copying the **reference** to the value and **not the actual value itself**, when we try and change something in the copied object, the orignal also changes. Since both the new object and original object are using the same references for their data, both objects get updated.

### Question 3

  The code logs `'Laisha is the hardest working person in the room.'` and `'Laisha is also the hardest working person in the room.'`. The reason, the `theHustler` **variable** didn't get updated to `'Paul'` as espected is because `theHustler` was declared using const, meaining once we assign a value to that **variable**, we cannot change or update it.
...
We can turn a **parameter** into a **rest parameter** by using *three dots* and the *name of the parameter* in the usual section for parameters in a function. The **rest parameter** must be the last **parameter** added to the function.
...
You can think of **scope** as a sort of container. You are able to use whatever's stored inside of the container, but you can't use anything that you haven't put in the container. You cannot reference or use data that is not within the scope of your code block.
...
const product = (num) => {
  ...
  return product // expected output: num * num /actual output: [Function: product]
};
```

The code snippet above displays a faulty understanding of **scope**. The programmer declares the **variable** `product` inside the **if statement**, yet they return the **variable** in the **parent code block**. Since the **variable** isn't declared in the **code block** it's returned in, the output of this function is faulty and we get a value that we were not expecting.

### Question 6

**Modules** in **JavaScript** allow you to group together blocks of code, or **functions** that have similar funcionality into a separate file. This code can then be accessed by other files in your directory by **exporting** and **importing** it into the file that you need it for.
...
We needed to make a copy of the array to make it a pure function because if we were altering the array itself, we would be **altering our input** and everytime we run the function, we'd get a **different result**. Array's are **mutable** so we can mutate an array. Because of this, we make a copy of it before we start making changes.

We don't want to mutate the original array because we might be *reusing* that array later on in our code, and if everytime we run the code the value for this array changes, our code will not perform how we expect it to, and will output *erroneous values*.
